 ...
 * </pre>
 * You should set a listener via {@link #addOnTabSelectedListener(OnTabSelectedListener)} to be
 * notified when any tab's selection state has been changed.
 * <p/>
 * If you're using a {@link ViewPager} together
 * with this layout, you can use {@link #setTabsFromPagerAdapter(PagerAdapter, int)} which will populate
...
public abstract class TabLayout extends HorizontalScrollView {
     ...
     */
    public void setScrollPosition(int oldPosition, int position, float positionOffset,
     ...
     * </p>
     *
     * @see #setTabsFromPagerAdapter(PagerAdapter, int)
...
//            }
        ...
        // Add Tabs, using the adapter's page titles
        setTabsFromPagerAdapter(adapter, curItem);
     ...
     */
    public void setTabsFromPagerAdapter(@NonNull PagerAdapter adapter, int curItem) {
        removeAllTabs();
        for (int i = 0, count = adapter.getCount(); i < count; i++) {
            addTab(newTab().setText(adapter.getPageTitle(i)), i == curItem);
        }
    }
    ...
    public void selectTab(Tab tab, boolean updateIndicator) {
        ...
        if (mSelectedTab == tab) {
            if (mSelectedTab != null) {
                if (mOnTabSelectedListeners != null) {
                    for (OnTabSelectedListener listener : mOnTabSelectedListeners) {
                        if (listener != null) {
                            listener.onTabReselected(mSelectedTab);
                        }
                    }
                }
            ...
            if (mSelectedTab != null && mOnTabSelectedListeners != null) {
                for (OnTabSelectedListener listener : mOnTabSelectedListeners) {
                    if (listener != null) {
                        listener.onTabUnselected(mSelectedTab);
                    }
                }
            }
            mSelectedTab = tab;
            if (mSelectedTab != null && mOnTabSelectedListeners != null) {
                for (OnTabSelectedListener listener : mOnTabSelectedListeners) {
                    if (listener != null) {
                        listener.onTabSelected(mSelectedTab);
                    }
                }
            }
        }
    ...
    class TabView extends LinearLayout implements OnLongClickListener {
        ...
        final void update() {
            ...
            if (custom != null) {
                ...
                custom.setSelected(tab.isSelected());
            } else {
